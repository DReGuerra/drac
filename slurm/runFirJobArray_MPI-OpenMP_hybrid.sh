...
#SBATCH --ntasks-per-node=24                            # MPI ranks per node; one rank per 8-core CCD on Fir
#SBATCH --cpus-per-task=8                               # OpenMP threads per MPI rank; one CCD per rank
#SBATCH --mem=0                                         # memory per node
#SBATCH --time=00-00:05:00                              # days-hh:mm:ss
#SBATCH --job-name=Au22KCK16                            # Name of the job
#SBATCH --output=slurm_%A_%a_tddft_Au22KCK16.out        # output file
#SBATCH --error=slurm_%A_%a_tddft_Au22KCK16.err         # error file
#SBATCH --array=1-1                                     # Job array with 1 task (can be increased for multiple tasks)
...
srun --cpu-bind=cores ./my_mpi_threaded_program # MPI-OpenMP hybrid program
